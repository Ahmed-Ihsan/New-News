"""
مصدر مواقع الأخبار التقنية — يجلب أحدث الأخبار من مواقع التقنية.
Tech News Sites source: fetches latest news from tech news sites via RSS/Atom.
"""
...
class TechNewsSource(NewsSource):
    ...
    def _parse_feed(self, xml: str, site: str) -> list[dict]:
        """تحليل RSS أو Atom واستخراج الأخبار المتعلقة بإطلاق الأدوات."""
        stories = []
        try:
            ...
            logger.warning(f"  ⚠️ فشل تحليل feed موقع {site}: {e}")
        return stories

    def _fetch_site(self, site: str, feed_url: str) -> list[dict]:
        """جلب وتحليل موقع أخبار تقنية واحد."""
        try:
            xml = fetch_html(feed_url)
            if not xml:
                return []
            return self._parse_feed(xml, site)
        except Exception as e:
            logger.warning(f"  ⚠️ فشل جلب موقع {site}: {e}")
            return []
    ...
    def fetch(self) -> list[dict]:
        ...
        try:
            with ThreadPoolExecutor(max_workers=4) as executor:
                futures = {
                    executor.submit(self._fetch_site, site, url): site
                    for site, url in self.SITES.items()
                }
                for future in as_completed(futures):
                    site = futures[future]
                    try:
                        site_stories = future.result()
                        for story in site_stories:
                            if len(stories) >= MAX_STORIES_PER_SOURCE:
                                break
                            stories.append(story)
                    except Exception as e:
                        logger.warning(f"  ⚠️ تخطي موقع {site}: {e}")
